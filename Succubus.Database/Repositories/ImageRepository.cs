        ...
        public Image GetImageFromCosplayer(string name)
        {
            try
            {
                var cosplayer = Context.Cosplayers
                    ...
                    .Where(x => x.Distance < 3 || x.x.Aliases.Split(',').ToList().Any(x => x.ToLowerInvariant().LevenshteinDistance(name) < 3))
                    .FirstOrDefault().x;
        ...
        public Image GetImageFromSet(string setName)
        {
            try
            {
                var set = Context.Sets
                   ...
                   .Where(y => y.Distance < 3 || y.y.Aliases.Split(',').ToList().Any(x => x.ToLowerInvariant().LevenshteinDistance(setName) < 3))
                   .FirstOrDefault().y;
